...
# PROJEÇÃO POR DIAS ÚTEIS — usada só pela Tendência de "Qualidade por módulo" (tabela_modulo)
# logo abaixo. Escopo desta mudança é SÓ esta tabela — não mexe no gráfico "Tendência dos
# módulos" (moduleTrendChart(), build_dash.py), que continua com sua régua própria (chips
# ligar/desligar top-6, sem seletor de janela). DECISÃO DE 08/09/2026: compara o volume PARCIAL
# do módulo neste mês (até hoje) com a média do que os últimos N meses FECHADOS tinham no MESMO
...
#                meses, trend3/trend6) — o seletor no front-end (mtSetWindow) escolhe qual
#                mostrar, sem recalcular nada. Escopo só desta tabela (ver comentário acima).
