...
.icon-btn{position:relative;width:38px;height:38px;border:1px solid var(--line);background:var(--surface-2);color:var(--text-1);border-radius:50%;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:16px;transition:background .15s,border-color .15s;flex:none}
...
.sidebar-top{display:flex;align-items:flex-start;justify-content:space-between;gap:10px;padding:18px 14px 4px}
.sidebar.collapsed .sidebar-top{flex-direction:column;align-items:center;justify-content:flex-start}
.sidebar-brand{display:flex;flex-direction:column;gap:6px;min-width:0;overflow:hidden}
.sidebar-brand img{height:52px;width:auto;display:block}
.sidebar-brand img.logo-dark{display:none}
:root[data-theme="dark"] .sidebar-brand img.logo-light{display:none}
:root[data-theme="dark"] .sidebar-brand img.logo-dark{display:block}
.sidebar-brand-sub{font-size:10.5px;font-weight:600;color:var(--text-3);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;letter-spacing:.2px}
...
.sidebar.collapsed .sidebar-profile{flex-direction:column;justify-content:center;gap:8px;padding:12px 8px}
...
.sidebar-profile-avatar .badge{position:absolute;top:-4px;right:-4px;min-width:14px;height:14px;padding:0 3px;border-radius:8px;color:#fff;font-size:9px;font-weight:700;display:flex;align-items:center;justify-content:center;border:2px solid var(--surface-1);font-family:inherit;line-height:1}
  ...
  <aside class="sidebar" id="sidebar">
    <div class="sidebar-top">
      <div class="sidebar-brand">
        ...
        <div class="sidebar-brand-sub">Setor de Desenvolvimento</div>
      </div>
...
// __trendAllMode: true quando a seleção atual é a visão agregada (todos os módulos), seja
// pelo padrão inicial da página, seja por clicar em "Selecionar todos". Liga a visão agregada
// dos 3 cartões (moduleSummaryCards); qualquer toque manual num chip ou "Limpar seleção"
// desliga, mesmo que o resultado ainda tenha todos marcados — DECISÃO DE 17/09/2026, a pedido
// da Jane: a visão geral é o padrão da página, mas uma seleção manual é sempre uma ação
// explícita que sai desse modo.
window.__trendAllMode=true;
function trendMods(){if(!window.__trendMods)window.__trendMods=new Set(DATA.mod_ano_a_ano.ordem);return window.__trendMods;}
...
function moduleSummaryCards(selMods,allMode){
  ...
  if(!r) return '';
  const c=trendColor(mod);
  const selo=r.small?'<span class="selo-amostra" title="Amostra pequena — módulo de baixo volume. O backtest que decidiu o método de projeção mostrou erro de até ~98% nesses casos; leia os números abaixo com cautela.">!</span>':'';
  const vp=r.var_pct_ano_anterior;
  ...
  return `<div class="mh-kpis" style="margin-top:4px">
    <div class="mh-kpi">
      <div class="mh-kpi-lbl">Estimativa de fechamento${selo}</div>
      <div class="mh-kpi-num" style="color:${c}">${Math.round(r.carga_projetada)}</div>
      <div class="mh-kpi-sub">${Math.round(r.criados_projetado)} novos + ${r.sobra_anterior} herdados de ${r.mes_anterior_lbl}</div>
    </div>
    <div class="mh-kpi">
      <div class="mh-kpi-lbl">Deve sobrar p/ ${r.mes_seguinte_lbl}${selo}</div>
      <div class="mh-kpi-num">${r.sobra_esperada!=null?Math.round(r.sobra_esperada):'N/D'}</div>
      <div class="mh-kpi-sub">${r.sobra_esperada_pct!=null?`${r.sobra_esperada_pct}% da carga projetada`:'sem histórico suficiente (menos de 1 mês fechado com carga)'}</div>
    </div>
    <div class="mh-kpi">
...
function funilPanel(){
  ...
  return `<div class="panel" style="border-left:5px solid var(--s1)">
    ...
    <div style="display:flex;flex-wrap:wrap;gap:2px;align-items:stretch">
      ...
      ${arrow('corte Backlog')}
      ${step(f.backlog,'backlog','ainda não entrou no fluxo',col('--text-3'))}
      ${arrow('corte Cancelado QA')}
    ...
    <div class="note" style="margin-top:16px"><b>Como ler:</b> os números do funil partem dos bugs CRIADOS na safra selecionada (ou no intervalo, em Acumulado) — é uma fotografia por mês de NASCIMENTO do card, não por mês de entrega. "Entregues" = quantos desses bugs (criados nesse mesmo período) já estão, agora, com status atual de Em produção/Done/Concluído. Por isso esse número pode ser MENOR que os "concluídos" do mesmo mês em "Bug por módulo": lá, "concluído" é o mês em que o card mudou pra Em produção, não importa quando ele foi criado — inclui bugs nascidos em meses anteriores e só resolvidos agora. Aqui no funil, um bug criado em agosto e entregue em setembro conta como entregue no funil de AGOSTO (o mês em que ele nasceu), não no de setembro — mesmo que o desenvolvimento tenha entregado ele em setembro. As duas leituras estão certas; medem populações diferentes de bugs (mês de criação × mês de conclusão).</div>
    <div class="note" style="margin-top:10px"><b>Leitura de capacidade:</b> o dev recebeu de verdade <b>${f.confirmados}</b> bugs confirmados (não ${f.total} — ${f.backlog} ainda em Backlog, ${f.descartados_qa} eram ruído de triagem que o QA barrou, ${f.pct_descarte}% do que passou do backlog, e <b>${f.cancelados_dev}</b> foram cancelados no próprio dev antes de virar carga real). Entregou <b>${f.entregues} (${f.pct_entrega}%)</b>${acum?' no período':' dentro do mês'}; a fila restante são ${f.fila} cards (${filaTxt}), a maior parte já adiantada. Velocidade: MTTR mediano de <b>${f.mttr_mediana} dias úteis</b> (média ${f.mttr_media}). Severidade do que chegou ao dev: ${sevTxt}. Concentração: ${modTxt}. Detecção ${acum?'do período':'da safra'} (todos os cards): ${detTxt}. Apontamento de horas em ${f.apont_cov[0]} de ${f.apont_cov[1]} cards. <b>O gargalo${acum?' do período':' do mês'} não foi o desenvolvimento</b> — foi a triagem deixando ${f.pct_descarte}% de ruído entrar.</div></div>`;
}
function prodBanner(){
...
function render(){
  ...
  document.getElementById('app').innerHTML=`
   ...
   <div class="panel">
     ...
     <div class="note"><b>Como ler:</b> marque os módulos nos chips e o gráfico soma os selecionados. Barra laranja = criados, verde = concluídos (1ª entrada em "Em produção"), linha vermelha = quantos passaram pro mês seguinte (saldo acumulado da seleção). Regra fixa: <b>saldo que passa para o próximo mês = cards no início do mês + cards criados − cards concluídos</b>. "Todos" dá o agregado geral; marcar só um grupo (ex.: os módulos BIM) dá o recorte daquele grupo. Cancelado QA, Cancelado Dev e cards ATUALMENTE parados em Impedimento Produto ou Backlog não contam — quem só passou por lá mas foi concluído ou avançou conta normalmente. <b>Por que "concluídos" aqui pode ser maior que "Entregues" no Diagnóstico do mês, no mesmo mês:</b> aqui o mês que conta é o da CONCLUSÃO (1ª entrada em Em produção) — não importa quando o card foi criado, então um bug nascido em meses anteriores e só resolvido agora entra nos concluídos deste mês. O Diagnóstico do mês faz o caminho inverso: só olha os bugs CRIADOS naquele mês, e "Entregues" é quantos desses já estão com status atual de entregue — um bug criado em agosto e entregue em setembro conta como entregue no funil de AGOSTO (mês em que nasceu), não no de setembro. As duas leituras estão certas; medem populações diferentes de bugs (mês de conclusão × mês de criação).</div>
   </div>
